...
medical_history <-
  read.csv(file = "input_files/2021_v2-5release_0510/clinical/PD_Medical_History.csv",
           stringsAsFactors = F, header = TRUE) %>%
  dplyr::select(-GUID)
medical_history_vars <- unique(colnames(medical_history)) %>% remove_idvars()

family_history <-
  read.csv(file = "input_files/2021_v2-5release_0510/clinical/Family_History_PD.csv",
           stringsAsFactors = F, header = TRUE) %>%
  dplyr::select(-GUID)
family_history_vars <- unique(colnames(family_history)) %>% remove_idvars()


genetic_status <-
  read.csv(file = "input_files/2021_v2-5release_0510/clinical/Clinically_Reported_Genetic_Status.csv",
           stringsAsFactors = F, header = TRUE) %>%
  dplyr::select(-c(GUID, visit_name, visit_month)) %>% 
  mutate_all(na_if,"") %>% 
  mutate_all(na_if,"Unknown/Not collected as enrollment criterion")

genetic_status_vars <- unique(colnames(genetic_status)) %>% remove_idvars()

medical_history_metadata <- list("medical_history" = medical_history_vars,
                              "family_history" = family_history_vars,
                              "genetic_status" = genetic_status_vars)

## Clinical Assessments ----
mds1 <-
  read.csv(file = "input_files/2021_v2-5release_0510/clinical/MDS_UPDRS_Part_I.csv",
           stringsAsFactors = F, header = TRUE) %>%
  dplyr::select(-GUID)
mds2 <-
  read.csv(file = "input_files/2021_v2-5release_0510/clinical/MDS_UPDRS_Part_II.csv",
           stringsAsFactors = F, header = TRUE) %>%
  dplyr::select(-GUID)
mds3 <-
  read.csv(file = "input_files/2021_v2-5release_0510/clinical/MDS_UPDRS_Part_III.csv",
           stringsAsFactors = F, header = TRUE) %>%
  dplyr::select(-GUID)
mds4 <-
  read.csv(file = "input_files/2021_v2-5release_0510/clinical/MDS_UPDRS_Part_IV.csv",
           stringsAsFactors = F, header = TRUE) %>%
  dplyr::select(-GUID)
mdsupdrs_vars <- unique(c(colnames(mds1), colnames(mds2), colnames(mds3), colnames(mds4))) %>% remove_idvars()

updrs <-
  read.csv(file = "input_files/2021_v2-5release_0510/clinical/UPDRS.csv",
           stringsAsFactors = F, header = TRUE) %>%
  dplyr::select(-GUID)
updrs_vars <- unique(colnames(updrs)) %>% remove_idvars()

upsit <-
  read.csv(file = "input_files/2021_v2-5release_0510/clinical/UPSIT.csv",
           stringsAsFactors = F, header = TRUE) %>%
  dplyr::select(-GUID)
upsit_vars <- unique(colnames(upsit)) %>% remove_idvars()

mmse <-
  read.csv(file = "input_files/2021_v2-5release_0510/clinical/MMSE.csv",
           stringsAsFactors = F, header = TRUE) %>%
  dplyr::select(-GUID)
mmse_vars <- unique(colnames(mmse)) %>% remove_idvars()

moca <-
  read.csv(file = "input_files/2021_v2-5release_0510/clinical/MOCA.csv",
           stringsAsFactors = F, header = TRUE) %>%
  dplyr::select(-c(GUID, code_education_12years_complete, education_12years_complete))
moca_vars <- unique(colnames(moca)) %>% remove_idvars()

modswb_adl <-
  read.csv(file = "input_files/2021_v2-5release_0510/clinical/Modified_Schwab___England_ADL.csv",
           stringsAsFactors = F, header = TRUE) %>%
  dplyr::select(-GUID)
modswb_adl_vars <- unique(colnames(modswb_adl)) %>% remove_idvars()

rem_mayo <-
  read.csv(file = "input_files/2021_v2-5release_0510/clinical/REM_Sleep_Behavior_Disorder_Questionnaire_Mayo.csv",
           stringsAsFactors = F, header = TRUE) %>%
  dplyr::select(-GUID)
rem_mayo_vars <- unique(colnames(rem_mayo)) %>% remove_idvars()

rem_sk <-
  read.csv(file = "input_files/2021_v2-5release_0510/clinical/REM_Sleep_Behavior_Disorder_Questionnaire_Stiasny_Kolster.csv",
           stringsAsFactors = F, header = TRUE) %>%
  dplyr::select(-GUID)
rem_sk_vars <- unique(colnames(rem_sk)) %>% remove_idvars()

sleep_scale <-
  read.csv(file = "input_files/2021_v2-5release_0510/clinical/Epworth_Sleepiness_Scale.csv",
           stringsAsFactors = F, header = TRUE) %>%
  dplyr::select(-GUID)
sleep_scale_vars <- unique(colnames(sleep_scale)) %>% remove_idvars()
...
mri <-
  read.csv(file = "input_files/2021_v2-5release_0510/clinical/MRI.csv",
           stringsAsFactors = F, header = TRUE) %>%
  dplyr::select(-GUID)
mri_vars <- unique(colnames(mri)) %>% remove_idvars()

datscan <-
  read.csv(file = "input_files/2021_v2-5release_0510/clinical/DaTSCAN_SBR.csv",
           stringsAsFactors = F, header = TRUE) %>%
  dplyr::select(-GUID)
datscan_vars <- unique(colnames(datscan)) %>% remove_idvars()

dti <-
  read.csv(file = "input_files/2021_v2-5release_0510/clinical/DTI.csv",
           stringsAsFactors = F, header = TRUE) %>%
  dplyr::select(-GUID)
...
smoking_alcohol <-
  read.csv(file = "input_files/2021_v2-5release_0510/clinical/Smoking_and_alcohol_history.csv",
           stringsAsFactors = F, header = TRUE) %>%
  dplyr::select(-GUID)
smoking_alcohol_vars <- unique(colnames(smoking_alcohol)) %>% remove_idvars()

caffeine <-
  read.csv(file = "input_files/2021_v2-5release_0510/clinical/Caffeine_history.csv",
           stringsAsFactors = F, header = TRUE) %>%
  dplyr::select(-GUID)
caffeine_vars <- unique(colnames(caffeine)) %>% remove_idvars()

pdq <-
  read.csv(file = "input_files/2021_v2-5release_0510/clinical/PDQ_39.csv",
           stringsAsFactors = F, header = TRUE) %>%
  dplyr::select(-GUID)
...
saveRDS(longitudinal_data, file = "data/Metadata/longitudinal_metadata.rds")
...
repaired_static <- coalesce(static_start_data2repair, records_wo_visit2repair)
...
sampdat <-
  read.csv(file = "input_files/2021_v2-5release_0510/amp_pd_participants.csv",
           stringsAsFactors = F, header = T) %>% 
  dplyr::select(participant_id, study)
case_control <-
  read.csv(file = "input_files/2021_v2-5release_0510/amp_pd_case_control.csv", 
           stringsAsFactors = F, header = TRUE)

# Data wrangling Demographics data

dem <-
  read.csv(file = "input_files/2021_v2-5release_0510/clinical/Demographics.csv",
           stringsAsFactors = F, header = TRUE) %>% 
...
demographic_vars <- list("demographic_vars" = 
                           unique(c(colnames(dem_v2),
                             colnames(sampdat),
                             colnames(case_control))))

saveRDS(dem_v2, file = "data/Metadata/demographics.rds")
...
saveRDS(sample_info, file = "data/Metadata/static_metdata.rds")
...
saveRDS(sample_info_long, file = "data/Metadata/longitudinal_metadata.rds")

#_______________________________________________________________________________
# Metadata Categories ----
#_______________________________________________________________________________

dt_list <- map(metadata_categories, as.data.table) %>% suppressWarnings()
metadata_categories_df <- 
  rbindlist(dt_list, fill = TRUE, idcol = T) %>% 
  pivot_longer(!.id) %>% 
  dplyr::rename(metadata_class = .id,
                metadata_subclass = name,
                metadata = value) %>% 
  drop_na(metadata) %>% 
  distinct()

saveRDS(metadata_categories, file = "data/Metadata/metadata_categories.rds")
saveRDS(metadata_categories_df, file = "data/Metadata/metadata_categories_df.rds")
